...
class ImgSaver:
        ...
        def save_image_deg(self, image, out_heat, out_delta, fileName, delta_height=10, delta_threshold = 50):


            # out_delta = out_delta.cpu().detach().numpy()
            # --------------------------Save segmented map
            ...
            for idx, lane in enumerate(score.lane_list):
                ...
                for idx2, height in enumerate(range(160, 710+1, 10)):
                    if lane[idx2] > 0:
                        ...
                        if idx > 10:
                            idx = 10
                        output_lane_image = cv2.circle(output_lane_image, (lane[idx2],height), 5, myColor.color_list[idx], -1)
                    idx2+=1
                idx+=1
            cv2.imwrite(key_fir_dir, output_key_image)
            cv2.imwrite(lane_fir_dir, output_lane_image)
            out_heat = out_heat.cpu().detach().numpy()
            cv2.imwrite(heat_fir_dir, (out_heat[:,:,1]+1)*50)
        ...
        def save_image_dir_deg(self, delta_model, heat_model, filePaths):
            for file in filePaths:
                ...
                gt_path = os.path.join("/home/ubuntu/Hgnaseel_SHL/Dataset/tuSimple/seg_label", *path_list[7:-1],"20.png")
                ...
                for idx, lane in enumerate(score.lane_list):
                    ...
                    for idx2, height in enumerate(range(160, 710+1, 10)):
                        if lane[idx2] > 0:
                            cls_img = cv2.circle(cls_img, (lane[idx2],height), 5, myColor.color_list[idx], -1)
                            gt_img = cv2.circle(gt_img, (lane[idx2],height), 5, myColor.color_list[idx], -1)
                            raw_img = cv2.circle(raw_img, (lane[idx2],height), 5, myColor.color_list[idx], -1)
                        idx2+=1
                    idx+=1


                dir_name = os.path.join(self.image_save_path, "check")
                os.makedirs(dir_name, exist_ok = True)
                fileName = "raw"
                fir_dir = os.path.join(dir_name,path_list[7] + "_" + path_list[8] + "_" + str(fileName)+".jpg")
                print("Raw Path = {}".format(fir_dir))
                cv2.imwrite(fir_dir, raw_img)
                fileName = "segmented"
                fir_dir = os.path.join(dir_name,path_list[8] + "_" + path_list[9] + "_" + str(fileName)+".jpg")
                print("Seg Path = {}".format(fir_dir))
                cv2.imwrite(fir_dir, cls_img)
                gtfileName = "ground_truth"
                fir_dir = os.path.join(dir_name,path_list[8] + "_" + path_list[9] + "_" + str(gtfileName)+".jpg")
